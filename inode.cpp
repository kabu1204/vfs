...
inode_mgmt::open(ushort inode_id, ushort _uid, ushort _gid) {
    ...
    inode& curr_inode = in_mem_inodes[inode_id];
    char p[8];
    std::string right = std::string(std::_itoa(curr_inode.mode,p,8));
...
std::pair<ushort, std::string> inode_mgmt::parse_path(std::string filepath, ushort _uid, ushort _gid) {
    ...
    if(filename!="/"){
        // 获取父目录对应的i节点
        auto res = path2inode_id(father_dir, this, _uid, _gid);
        if(!res.second){
...
size_t inode_mgmt::getsizeof(std::string _filepath, ushort _uid, ushort _gid) {
     ...
     *   获取文件的大小(Bytes)
     */
    auto res = path2inode_id(_filepath, this, _uid, _gid);
    if(!res.second){
        std::cerr<<"Failed to get "<<_filepath<<"\'s inode_id!\n";
        return -1;
    }
    return _getsizeof(res.first);
}

size_t inode_mgmt::_getsizeof(ushort inode_id) {
    /*
     * 内部方法：
     *   获取指定i节点号所指向文件的大小
     *   i节点未分配时返回-1
